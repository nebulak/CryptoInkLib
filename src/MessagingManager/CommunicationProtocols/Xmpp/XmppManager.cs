...
namespace CryptoInkLib
{
	...
	public class XmppManager
	{
		...
		public PresenceManager m_PresenceManager;
		...
		/// </summary>
		private Presence m_OwnPresence;
		...
		{
			this.m_PresenceManager.update (_presence);
		}


		public void sendMessage(string sReceiverId, string sMessage)
		{
			...
			//save message
			m_ConversationManager.addMessage (ECommunicationProtocol.XMPP, sMessage, m_OwnJid, sReceiverId);
		}


		public void sendPgpMessage()
		{
			//TODO: implement PGPmessage
...
//			m_OtrSessionManager.GetExtraSymmetricKey (e.GetSessionID ());
		...
		private void sendXmppMessage(string sReceiverJid, string sMessage)
		{
			m_ClientConnection.Send(new agsXMPP.protocol.client.Message(new Jid(sReceiverJid), agsXMPP.protocol.client.MessageType.chat, sMessage));
		...
		public List<ContactPresence> getPresenceList()
		{
			return m_PresenceManager.m_PresenceList;
		}
		...
		{
			//check if user is authenticated
			if (m_ClientConnection.Authenticated && m_ClientConnection.XmppConnectionState == XmppConnectionState.Authenticated) {
				m_Logger.log (ELogLevel.LVL_INFO, "isConnected: Xmpp user is logged in", m_sModuleName);
				return true;
			}
			return false;
		}
